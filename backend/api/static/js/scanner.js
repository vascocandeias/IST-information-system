...
function printLesson(lesson) {
    return `
    <li> ${lesson.course.acronym}  - ${lesson.info} - ${lesson.course.name}</li>
    `;
}
function lessons(events) {
    return `
    <p>Lessons</p>
    ${events.filter(event => event.type == "LESSON").map(printLesson).join("")}
    </ul>
    `;
}

function hasOtherEvents() {
...
document.getElementById("divDestiny").innerHTML = `${roomTemplate(room)}`;
...
scanner.addListener('scan', function (content) {
    jQuery.getJSON(content, function(data, status){
        ...
        document.getElementById("divDestiny").innerHTML = "Other: " + data.name;
